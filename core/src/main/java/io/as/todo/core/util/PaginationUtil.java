...
public final class PaginationUtil
{
    ...
    public static HttpHeaders generateSearchPaginationHttpHeaders(String query, Page<?> page, String baseUrl) throws URISyntaxException
    {
        HttpHeaders headers = new HttpHeaders();
        headers.add("X-Total-Count", "" + page.getTotalElements());
        String link = "";
    ...
    public static HttpHeaders generateSearchPaginationHttpHeaders(Map<String, String> fields, Page<?> page, String baseUrl) throws URISyntaxException
    {
        ...
        if ((page.getNumber() + 1) < page.getTotalPages())
        {
            link = "<" + (new URI(baseUrl + "?page=" + (page.getNumber() + 1) + "&size=" + page.getSize())).toString() + query + ">; rel=\"next\",";
        }
        // prev link
        if ((page.getNumber()) > 0)
        {
            link += "<" + (new URI(baseUrl + "?page=" + (page.getNumber() - 1) + "&size=" + page.getSize())).toString() + query + ">; rel=\"prev\",";
        }
        // last and first link
        int lastPage = 0;
        if (page.getTotalPages() > 0)
        ...
        link += "<" + (new URI(baseUrl + "?page=" + lastPage + "&size=" + page.getSize())).toString() + query + ">; rel=\"last\",";
        link += "<" + (new URI(baseUrl + "?page=" + 0 + "&size=" + page.getSize())).toString() + query + ">; rel=\"first\"";
        headers.add(HttpHeaders.LINK, link);
        return headers;
    }
